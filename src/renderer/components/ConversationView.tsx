...
import {
  ...
  SpinnerGap, ArrowCounterClockwise, Square, BookmarkSimple,
} from '@phosphor-icons/react'
import { useSessionStore, isBookmarked, toggleBookmark } from '../stores/sessionStore'
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px]" style={{ color: colors.textTertiary }}>
        Press <strong style={{ color: colors.textSecondary }}>{useSessionStore.getState().staticInfo?.platform === 'win32' ? 'Ctrl + Space' : '⌥ + Space'}</strong> to show/hide this overlay
      </span>
    </div>
  )
}
...
function CodeBlockActions({ code, language }: { code: string; language?: string }) {
  ...
  const handleRunInTerminal = () => {
    const tab = useSessionStore.getState().tabs.find(
      (t) => t.id === useSessionStore.getState().activeTabId
    )
    window.clui.openInTerminal(
  ...
  return (
    ...
    >
      {language && (
        <span className="text-[10px] font-mono mr-auto" style={{ color: colors.textTertiary }}>
          {language}
        </span>
      )}
      <button
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <motion.button
      ...
      onClick={handleCopy}
      className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[11px] cursor-default flex-shrink-0"
      style={{
        background: copied ? colors.statusCompleteBg : 'transparent',
        color: copied ? colors.statusComplete : colors.textTertiary,
        border: 'none',
...
function BookmarkButton({ messageId }: { messageId: string }) {
  const [bookmarked, setBookmarked] = useState(() => isBookmarked(messageId))
  const colors = useColors()
  ...
  return (
    <motion.button
      ...
      exit={{ opacity: 0 }}
      transition={{ duration: 0.12 }}
      onClick={() => setBookmarked(toggleBookmark(messageId))}
      className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[11px] cursor-default flex-shrink-0"
      style={{
        background: bookmarked ? colors.accentLight : 'transparent',
        color: bookmarked ? colors.accent : colors.textTertiary,
        border: 'none',
      }}
      title={bookmarked ? 'Remove bookmark' : 'Bookmark'}
    >
      <BookmarkSimple size={11} weight={bookmarked ? 'fill' : 'regular'} />
    </motion.button>
  )
}
...
function InterruptButton({ tabId }: { tabId: string }) {
  ...
  return (
    <motion.button
      ...
      transition={{ duration: 0.12 }}
      onClick={handleStop}
      className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[11px] cursor-default flex-shrink-0 transition-colors"
      style={{
...
function ImageCard({ src, alt, colors }: { src?: string; alt?: string; colors: ReturnType<typeof useColors> }) {
  ...
  if (failed || !src) {
    return (
      <button
        type="button"
        className="inline-flex items-center gap-1.5 my-1 px-2.5 py-1.5 rounded-md text-[12px] cursor-default"
  ...
  return (
    <button
      type="button"
      className="block my-2 rounded-lg overflow-hidden border text-left cursor-default"
...
const AssistantMessage = React.memo(function AssistantMessage({
  ...
  const markdownComponents = useMemo(() => ({
    ...
    pre: ({ children }: any) => {
      // Extract code text by walking the React element tree to find raw strings
      ...
      const codeChild = React.Children.toArray(children).find(
        (c: any) => c?.type === 'code' || c?.props?.className?.startsWith?.('language-')
      ) as any
      const language = (codeChild?.props?.className || '').replace('language-', '')
      const codeText = extractText(codeChild) || extractText(children)
      return (
        <div className="my-2 rounded-lg overflow-hidden" style={{ border: `1px solid ${colors.toolBorder}` }}>
          <CodeBlockActions code={codeText.trimEnd()} language={language} />
          <pre style={{ margin: 0, padding: '12px', background: colors.codeBg, overflowX: 'auto', fontSize: 12 }}>
            {children}
          </pre>
        </div>
      )
  ...
  const inner = (
    <div className="group/msg relative">
      ...
      {message.content.trim() && (
        <div className="absolute bottom-0 right-0 opacity-0 group-hover/msg:opacity-100 transition-opacity duration-100 flex items-center gap-0.5">
...
function ToolGroup({ tools, skipMotion }: { tools: Message[]; skipMotion?: boolean }) {
  ...
  if (isOpen) {
    const inner = (
      <div className="py-1">
        ...
        {!hasRunning && (
          <div
            className="flex items-center gap-1 cursor-default mb-1.5"
  ...
  const inner = (
    <div
      className="flex items-start gap-1 cursor-default py-[2px]"
