...
export function PermissionCard({ tabId, permission, queueLength = 1 }: Props) {
  ...
  // Keyboard-driven permission handling
  React.useEffect(() => {
    if (responded) return
    const handler = (e: KeyboardEvent) => {
      ...
      // Enter = first allow option
      if (e.key === 'Enter') {
        e.preventDefault()
        const allow = options.find((o) =>
          o.kind === 'allow' || o.label.toLowerCase().includes('allow') || o.label.toLowerCase().includes('yes')
        )
        if (allow) handleOption(allow.id)
        return
      }

      // Escape = first deny option
      if (e.key === 'Escape') {
        e.preventDefault()
        const deny = options.find((o) =>
          o.kind === 'deny' || o.label.toLowerCase().includes('deny')
          || o.label.toLowerCase().includes('no') || o.label.toLowerCase().includes('reject')
        )
        if (deny) handleOption(deny.id)
        return
      }

      // Number keys 1-9 = select option by index
      const num = parseInt(e.key, 10)
      if (num >= 1 && num <= options.length) {
        e.preventDefault()
        handleOption(options[num - 1].id)
      }
    }
    window.addEventListener('keydown', handler)
    return () => window.removeEventListener('keydown', handler)
  }, [permission.questionId, responded])
    ...
    >
      ...
      >
        ...
        <div className="px-3 py-2.5">
          ...
          {inputPreview && (
            <pre
              className="text-[11px] leading-[1.5] px-2.5 py-2 rounded-lg overflow-x-auto whitespace-pre-wrap break-all mb-2 font-mono"
              style={{
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {permission.options.map((opt, idx) => {
              ...
              const shortcutHint = isAllow ? '↵' : isDeny ? 'esc' : `${idx + 1}`
              ...
              return (
                <button
                  ...
                  disabled={responded}
                  className="text-[11px] font-medium px-3 py-1.5 rounded-full transition-colors cursor-default disabled:opacity-40 disabled:cursor-not-allowed"
                  style={{
                    background: bg,
                    color: textColor,
                ...
                >
                  {opt.label}
                  <span className="ml-1 text-[9px] opacity-50">{shortcutHint}</span>
                </button>
              )
            })}
