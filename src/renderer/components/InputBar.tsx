...
export function InputBar() {
  ...
  const executeCommand = useCallback((cmd: SlashCommand) => {
    switch (cmd.command) {
      ...
      case '/project': {
        window.clui.selectDirectory().then((dir) => {
          if (dir) {
            useSessionStore.getState().setBaseDirectory(dir)
            addSystemMessage(`Working directory changed to: ${dir}`)
          }
        })
        break
      }
      case '/history': {
        addSystemMessage('Use the clock icon (🕐) in the tab strip to browse and resume previous sessions.')
        break
      }
      case '/help': {
        const lines = [
          ...
          '/project — Change working directory',
