...
export interface CluiAPI {
  ...
  takeScreenshot(mode?: 'fullscreen' | 'region'): Promise<Attachment | null>
  ...
  setShortcuts(shortcuts: Record<string, string>): Promise<{ ok: boolean }>
  onScreenshotAsk(callback: (attachment: Attachment) => void): () => void
  ...
  /** Send a native OS notification (only shows when overlay is hidden) */
...
const api: CluiAPI = {
  ...
  takeScreenshot: (mode?: 'fullscreen' | 'region') => ipcRenderer.invoke(IPC.TAKE_SCREENSHOT, mode || 'region'),
  ...
  setShortcuts: (shortcuts) => ipcRenderer.invoke(IPC.SET_SHORTCUTS, shortcuts),
  onScreenshotAsk: (callback) => {
    const handler = (_e: Electron.IpcRendererEvent, attachment: Attachment) => callback(attachment)
    ipcRenderer.on(IPC.SCREENSHOT_ASK, handler)
    return () => ipcRenderer.removeListener(IPC.SCREENSHOT_ASK, handler)
  },
  ...
  notify: (title, body, urgency) =>
    ipcRenderer.send('clui:notify', { title, body, urgency }),
