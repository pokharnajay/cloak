...
Cloak runs entirely on your local machine. Key security properties:
...
- **Preload isolation** — the renderer has no direct access to Node.js APIs; all IPC goes through a typed `window.cloak` bridge.
...
No other network connections are made by Cloak itself. The `claude` CLI may make its own connections as part of normal operation.
